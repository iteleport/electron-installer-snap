...
if (!process.env['FAST_TESTS_ONLY']) {
  test.serial('creates a snap', t => {
    let snapPath
    return snap({ src: path.join(__dirname, 'fixtures', 'app-with-asar'), base: 'core18' })
      .then(path => {
  ...
  test.serial('creates a snap in a custom output directory', t => {
    ...
    return fs.mkdirs(destDir)
      .then(() => snap({ src: path.join(__dirname, 'fixtures', 'app-with-asar'), dest: destDir, base: 'core18' }))
      .then(path => {
        t.truthy(path, 'snap returns a truthy value')
        snapPath = path
